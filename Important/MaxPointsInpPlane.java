    ...
    public int maxPoints(int[][] points){
        ...
        for(int i = 0; i< n && !set.contains(points[i][0] + "-" + points[i][1]) ; i++){
            ...
            for(int j = i+1; j<n; j++){
                ...
                map.put(slope , map.getOrDefault(slope,1)+1);  // increment by one if cause one point exist already if there is already a value in slope, else add two points
