public class Solution {
    public ArrayList<String> fullJustify(ArrayList<String> a, int b) {
        ...
        int maxLengthTillIndexInLine=0, currentWordIndex=0, wordCount=0, totalLengthInThisLine=0;
        while(currentWordIndex<l) {
            //if not first line we need a space minimum before adding next word
            if(wordCount != 0)
                maxLengthTillIndexInLine++;
            ...
            if(maxLengthTillIndexInLine<=b) {
                ...
                if(wordCount==1){
                    ...
                    while(tempWordCount>1) {
                        int spacesAfterWord = (totalSpaces%(tempWordCount-1)>0)?(totalSpaces/(tempWordCount-1)+1):(totalSpaces/(tempWordCount-1));
